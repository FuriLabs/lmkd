...
int init_psi_monitor(enum psi_stall_type stall_type,
    ...
    if (fd < 0) {
        g_printerr("No kernel psi monitor support (errno=%d)", errno);
        return -1;
    }
    ...
    case (PSI_FULL):
        ...
        break;
    default:
        g_printerr("Invalid psi stall type: %d", stall_type);
        errno = EINVAL;
        goto err;
    }

    if (res >= (ssize_t)sizeof(buf)) {
        g_printerr("%s line overflow for psi stall type '%s'",
    ...
    if (res < 0) {
        g_printerr("%s write failed for psi stall type '%s'; errno=%d",
...
int register_psi_monitor(int epollfd, int fd, void* data) {
    ...
    if (res < 0)
        g_printerr("epoll_ctl for psi monitor failed; errno=%d", errno);
    return res;
}
